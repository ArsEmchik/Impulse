package blocks {
	...
	public class CableView extends MovieClip 
	{
		...
		public function CableView() 
		{
			...
			nameVector.push("ПТРК-1(1 полукомплект)","ПТРК-2(1 полукомплект)","ПТРК-3(1 полукомплект)","ПТРК-1(2 полукомплект)",
							"ПТРК-2(2 полукомплект)","ПТРК-3(2 полукомплект)");
			var i: int;
			...
			for (i=0; i<nameVector.length; i++)
			{
				...
				if (ModeInfo.modeInfo.Czk86Channels[i]!=0)
				{
					...
					else resultString+=(nameVector[i]+" ЦК48: "+ModeInfo.modeInfo.Czk86Channels[i]);
					addToResult=true;
				}
				if (addToResult)
					resultString+="\r\n";
